...
public class TelaClientes extends JFrame {
    ...
    public TelaClientes() {
        ...
        modelo = new DefaultTableModel(new String[]{
                "ID", "Nome", "CPF", "Telefone", "Email", "Endereço", "Nascimento"
        }, 0);
        tabela = new JTable(modelo);
        JScrollPane scroll = new JScrollPane(tabela);
        scroll.setBounds(20, 20, 740, 200);
        add(scroll);

        JButton btnAdicionar = new JButton("Adicionar");
        btnAdicionar.setBounds(20, 240, 120, 30);
        btnAdicionar.addActionListener(e -> {
            System.out.println("Clicou em Adicionar"); // debug
            new FormularioCliente(this, null, dados -> modelo.addRow(dados));
        });
        ...
        btnAlterar.addActionListener(e -> {
            ...
            if (linha != -1) {
                String[] dadosOriginais = new String[7];
                for (int i = 0; i < 7; i++) {
                    dadosOriginais[i] = (String) modelo.getValueAt(linha, i);
                }

                new FormularioCliente(this, dadosOriginais, dadosAtualizados -> {
                    for (int i = 0; i < 7; i++) {
